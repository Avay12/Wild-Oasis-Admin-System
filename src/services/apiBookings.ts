...
export async function getBookings({ filter, sortBy, page }: Props) {
  let query = supabase
    .from("bookings")
    .select(
      "id, created_at, startDate, endDate, numNights, numGuests, status, totalPrice, cabins(name),guests!bookings_guestId_fkey(fullName,email)",
      { count: "exact" }
    );
...
export async function getBooking(id: string) {
  const { data, error } = await supabase
    .from("bookings")
    .select("*, cabins(*), guests!bookings_guestId_fkey(*)")
    .eq("id", +id)
    .single();
